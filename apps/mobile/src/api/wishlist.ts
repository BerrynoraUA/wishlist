...
export async function getMyWishlists(params: PaginationParams = {}): Promise<Wishlist[]> {
  const {
    data: { session },
  } = await supabase.auth.getSession();

  return getWishlists((query) => query.eq("user_id", session?.user?.id), params);
}
...
export async function getFriendsWishlistsDiscover(
  ...
  const { data, error } = await supabase.rpc("get_friends_wishlists_discover", {
    p_skip: skip,
    p_take: take,
  });
...
export async function updateWishlist(
  ...
  if (updates.visibility !== undefined) dbUpdates.visibility_type = updates.visibility;
  if (updates.imageUrl !== undefined) dbUpdates.image_url = updates.imageUrl;
  if (updates.accent !== undefined) dbUpdates.accent_type = updates.accent;
  if (updates.event_date !== undefined) dbUpdates.event_date = updates.event_date as any;
