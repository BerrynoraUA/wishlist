...
test.describe("Auth redirects — unauthenticated user", () => {
  ...
  test("accessing /friends without auth redirects to /login", async ({
    page,
  }) => {
  ...
  test("accessing /secret-santa without auth redirects to /login", async ({
    page,
  }) => {
  ...
  test("accessing /settings without auth redirects to /login", async ({
    page,
  }) => {
  ...
  test("accessing /discover without auth redirects to /login", async ({
    page,
  }) => {
  ...
  test("/share route is NOT redirected — middleware skips it", async ({
    page,
  }) => {
    ...
    await expect(
      page.getByText("Invalid share link."),
...
test.describe("Auth redirects — authenticated user", () => {
  ...
  test("authenticated user visiting / is redirected to /home", async ({
    page,
  }) => {
    await page.goto("/");
    await page.waitForURL(/\/home/, { timeout: 10_000 });
