...
test.describe("Wishlist CRUD interactions", () => {
  ...
  test("owner sees Open item menu button on each card", async ({ page }) => {
    ...
    await expect(page.getByRole("button", { name: "Open item menu" }).first()).toBeVisible();
  });

  test("clicking item menu button opens dropdown with Edit and Delete", async ({ page }) => {
    await page.getByRole("button", { name: "Open item menu" }).first().click();
    // Dropdown renders plain <button><span>Edit</span></button> buttons
    await expect(
      page.locator('[class*="dropdown"] button').filter({ hasText: "Edit" }).first(),
    ).toBeVisible();
    await expect(
      page.locator('[class*="dropdown"] button').filter({ hasText: "Delete" }).first(),
    ).toBeVisible();
  });

  // ── Edit Item ────────────────────────────────────────────────────────────

  test("clicking Edit in item dropdown opens EditItemModal", async ({ page }) => {
    await page.getByRole("button", { name: "Open item menu" }).first().click();
  ...
  test("EditItemModal pre-populates the item name field", async ({ page }) => {
    ...
    const nameInput = page
      .locator(
        'input[placeholder*="headphones"], input[placeholder*="item name"], input[placeholder*="Noise"]',
      )
      .first();
  ...
  test("EditItemModal Cancel button closes the modal", async ({ page }) => {
    ...
    await expect(page.getByRole("heading", { name: /edit item/i })).not.toBeVisible();
  });
  ...
  test("clicking Delete in item dropdown opens DeleteConfirmModal", async ({ page }) => {
    await page.getByRole("button", { name: "Open item menu" }).first().click();
  ...
  test("Delete Item modal Cancel closes without removing items", async ({ page }) => {
    await page.getByRole("button", { name: "Open item menu" }).first().click();
    await page.locator('[class*="dropdown"] button').filter({ hasText: "Delete" }).first().click();
    await expect(page.getByRole("heading", { name: /delete item/i })).toBeVisible({
      timeout: 5_000,
    ...
    await page.getByRole("button", { name: /cancel/i }).click();
    await expect(page.getByRole("heading", { name: /delete item/i })).not.toBeVisible();
  ...
  test("Wishlist actions button is visible in the header", async ({ page }) => {
    ...
    await expect(page.getByRole("button", { name: "Wishlist actions" })).toBeVisible({
      timeout: 10_000,
    });
  });

  test("clicking Wishlist actions reveals Edit and Delete options", async ({ page }) => {
  ...
  test("clicking Edit in wishlist actions opens EditWishlistModal", async ({ page }) => {
    await page.getByRole("button", { name: "Wishlist actions" }).click();
    await page
      .locator('[class*="menuDropdown"] button, [class*="dropdown"] button')
      .filter({ hasText: "Edit" })
      .first()
      .click();
    await expect(page.getByRole("heading", { name: /edit wishlist/i })).toBeVisible({
      timeout: 5_000,
    });
  });
  ...
  test("EditWishlistModal name field is pre-populated", async ({ page }) => {
    ...
    const nameInput = page
      .locator(
        'input[placeholder*="Birthday"], input[placeholder*="wishlist"], input[placeholder*="Birthday Wishes"]',
      )
      .first();
    const fallback = page.locator('input[type="text"]').first();
    const input = (await nameInput.count()) > 0 ? nameInput : fallback;
  ...
  test("EditWishlistModal has privacy options", async ({ page }) => {
    ...
    await expect(modal.getByRole("heading", { name: /edit wishlist/i })).toBeVisible({
      timeout: 5_000,
    });
  ...
  test("EditWishlistModal Cancel button closes the modal", async ({ page }) => {
    ...
    await page.getByRole("button", { name: /cancel/i }).click();
    await expect(page.getByRole("heading", { name: /edit wishlist/i })).not.toBeVisible();
  });

  // ── Delete Wishlist ──────────────────────────────────────────────────────

  test("clicking Delete in wishlist actions opens DeleteConfirmModal", async ({ page }) => {
    await page.getByRole("button", { name: "Wishlist actions" }).click();
    await page
      .locator('[class*="menuDropdown"] button, [class*="dropdown"] button')
      .filter({ hasText: "Delete" })
  ...
  test("Delete Wishlist modal Cancel closes without navigating away", async ({ page }) => {
    await page.getByRole("button", { name: "Wishlist actions" }).click();
    await page
      .locator('[class*="menuDropdown"] button, [class*="dropdown"] button')
      .filter({ hasText: "Delete" })
  ...
  test("inline edit pencil button is visible for the owner", async ({ page }) => {
    // WishlistHeader renders aria-label="Inline edit wishlist" on the Pencil button
    await expect(page.getByRole("button", { name: "Inline edit wishlist" })).toBeVisible({
      timeout: 10_000,
    });
  });

  test("clicking inline edit button shows title input field", async ({ page }) => {
    await page.getByRole("button", { name: "Inline edit wishlist" }).click();
    // WishlistHeader renders <input placeholder="Wishlist title" />
    await expect(page.locator('input[placeholder="Wishlist title"]')).toBeVisible({
      timeout: 3_000,
    });
  });
  ...
  test("pressing Escape cancels inline editing", async ({ page }) => {
    ...
    await page.keyboard.press("Escape");
    await expect(page.locator('input[placeholder="Wishlist title"]')).not.toBeVisible({
      timeout: 3_000,
    });
  });

  test("inline edit input is pre-filled with current wishlist title", async ({ page }) => {
    await page.getByRole("button", { name: "Inline edit wishlist" }).click();
