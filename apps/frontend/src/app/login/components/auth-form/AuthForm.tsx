...
export function AuthForm({ mode, redirectTo, initialEmail, onLoginSuccess }: Props) {
  const t = useGT();
    ...
    if (!emailRegex.test(email)) {
      setError(
        t("Please enter a valid email address.", {
          $id: "login.form.error.invalidEmail",
        }),
      );
      return;
    }
    ...
    if (password.length < 6) {
      setError(
        t("Password must be at least 6 characters.", {
          $id: "login.form.error.passwordTooShort",
        }),
      );
      return;
    }
      ...
      if (password !== confirmPassword) {
        setError(
          t("Passwords do not match.", {
            $id: "register.form.error.passwordMismatch",
          }),
        );
        return;
      }
      ...
      <form onSubmit={handleSubmit} className={styles.form}>
        ...
        {!isLogin && (
          <>
            <label className={styles.label}>
              {t("Confirm password", {
                $id: "register.form.label.confirmPassword",
              })}
              <span className={styles.required}>*</span>
            </label>
            <div className={styles.passwordWrapper}>
