 ...
 *
 * Metro cannot resolve fully dynamic requires, so locales are enumerated explicitly.
 * Each entry is a thunk rather than a bare `require` so that only the active locale's
 * bundle is materialised — requiring all 41 up front cost ~1.7 MB of object allocation
 * on every cold start.
 */
