/**
 * Client-side creation of localized notifications (native).
 *
 * Renders the notification text in each recipient's own language from the pre-translated
 * GT catalogs, then writes it through the SECURITY DEFINER `create_notification` RPC.
 * Best-effort: a failure here never propagates into the action that triggered it.
 */
...
async function getRecipientLocales(userIds: string[]): Promise<Map<string, string>> {
  ...
  const { data, error } = await supabase.rpc("get_preferred_locales", {
    p_user_ids: userIds,
  });
...
export async function notifySecretSantaInvites(
  ...
  try {
    ...
    debugError("[notifications] failed to notify secret santa invites", error);
  }
}
...
export async function notifyWishlistAccessGranted(
  ...
  try {
    const { data } = await supabase.from("wishlist").select("title").eq("id", wishlistId).single();
    ...
    debugError("[notifications] failed to notify wishlist access", error);
  }
}
...
export async function notifyNewWishlist(wishlistId: string, title: string): Promise<void> {
  try {
    const { data, error } = await supabase.rpc("get_wishlist_friends_to_notify", {
    ...
    debugError("[notifications] failed to notify friends about new wishlist", error);
  }
}

/**
 * Creates many localized notifications (fan-out). Each recipient gets the text in their own
 * language. Locales are looked up in a single batched RPC.
 */
export async function createLocalizedNotifications(
  ...
  try {
    ...
    debugError("[notifications] failed to create localized notification", error);
  }
}
