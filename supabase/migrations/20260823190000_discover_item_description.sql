-- The Discover feed builds its items as JSON and never included the description, so the
-- item detail modal there had nothing to show — the same item opened from a wishlist
-- shows one. Both feed functions now carry it.
