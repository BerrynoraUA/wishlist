...
- **`wishlists` #1** points at the owner's own **Reserved** counter. That stat is items
  on *your* lists that friends have claimed — not gifts you claimed for someone else.
  The owner sees the *count*, but by default never *which* items: `show_own_reservations`
